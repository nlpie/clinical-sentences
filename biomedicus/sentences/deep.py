...
class BiLSTMSentenceModel(SentenceModel):
    ...
    def _build_model(self) -> keras.models.Model:
        ...
        normed = keras.layers.BatchNormalization()(context)
        ...
        return keras.models.Model(inputs=inputs, outputs=[logits, context])

    def build_layers(self):
        chars_embedding = None
        char_input = None
        if self.use_chars:
            char_input = keras.layers.Input(
                shape=(self.sequence_length, self.word_length),
                dtype='int32',
                name='char_input')

            char_embedding = keras.layers.TimeDistributed(
                keras.layers.Embedding(input_dim=self.chars,
            ...
            if self.char_mode == 'cnn':
                ...
                char_cnn = keras.layers.TimeDistributed(
                    keras.layers.Conv1D(
                        cnn_filters,
                ...
                chars_embedding = keras.layers.TimeDistributed(
                    keras.layers.GlobalMaxPooling1D(name='char_pooling'),
                ...
                chars_embedding = keras.layers.TimeDistributed(
                    keras.layers.Bidirectional(
                        keras.layers.LSTM(units=char_lstm_hidden_size,
        ...
        if self.use_words:
            word_input = keras.layers.Input(shape=(self.sequence_length,),
                                            dtype='int32',
                                            name='word_input')

            if self.word_embeddings is not None:
                word_embedding = keras.layers.Embedding(input_dim=self.words,
            ...
            if chars_embedding is not None:
                ...
                if self.concatenate_words_chars:
                    word_embedding = keras.layers.Concatenate(
                        name="word_representation"
                    )([chars_embedding, word_embedding])
                else:
                    word_embedding = keras.layers.Add(
                        name="word_representation"
                    )([chars_embedding, word_embedding])
        ...
        word_embedding = keras.layers.BatchNormalization()(word_embedding)
        context = keras.layers.Bidirectional(
            keras.layers.LSTM(self.lstm_hidden_size,
    ...
    def compile_model(self,
                      optimizer: Union[AnyStr, keras.optimizers.Optimizer]):
